...
export async function sendReceiptEmail({
	...
	const productList = purchasedItems
		.map(
			(item) =>
				`- ${item.name} (${item.variantName}, ${item.pageCount} pages) x${item.quantity}`,
		)
		.join('\n')
	const downloadList = purchasedItems
		.flatMap((item) =>
			item.downloads.length > 0
				? item.downloads.map(
						(download) =>
							`- ${item.name} (${item.variantName}): ${download.label}`,
					)
				: [
	...
	await sendEmail({
		...
		text: [
			...
			'',
			'If you have any trouble accessing your files, reply to cameronbriggs8706@gmail.com or text me at 208.312.5052.',
		].join('\n'),
	})
}
