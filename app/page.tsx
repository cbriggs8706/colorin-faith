...
import { CustomProductFeature } from '@/components/custom-product-feature'
...
export default async function Home({
	...
	const [products, siteContent, customProduct, resolvedSearchParams] = await Promise.all([
		getProducts(),
		getSiteContent(),
		getCustomProduct(),
		searchParams,
	...
	const featuredProduct =
		visibleProducts.find((product) => product.featured) ??
		visibleProducts[0] ??
		null
		...
		<div className="flex flex-col gap-10 py-4 sm:gap-14 sm:py-6">
				...
				{featuredProduct ? (
					...
					<section className="card-surface rounded-[2rem] px-5 py-8 sm:px-8">
						<p className="text-base font-bold text-slate-700">
							{siteContent.homepage.shop.emptyState}
						</p>
					</section>
				)}
			</section>
			...
			<section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
				<div className="card-surface rounded-[2rem] px-5 py-6 sm:px-7">
					<p className="pill-label w-fit text-[var(--brand-coral)]">
						{siteContent.homepage.howItWorks.eyebrow}
					</p>
					<h2 className="section-title mt-3 text-3xl font-extrabold text-[var(--brand-ink)]">
						{siteContent.homepage.howItWorks.title}
					</h2>
				...
				<div className="gradient-soft rounded-[2rem] px-5 py-6 shadow-[0_24px_60px_rgba(32,48,66,0.12)] sm:px-7">
					<p className="pill-label w-fit text-[var(--brand-ink)]">
						{siteContent.homepage.newsletter.eyebrow}
					</p>
					<h2 className="section-title mt-3 text-3xl font-extrabold text-[var(--brand-ink)]">
						{siteContent.homepage.newsletter.title}
					</h2>
					<p className="mt-3 max-w-xl text-sm leading-6 text-slate-700 sm:text-base">
						{siteContent.homepage.newsletter.description}
					</p>
					<div className="mt-6">
						<NewsletterForm
							heading={siteContent.homepage.newsletter.formHeading}
							subheading={siteContent.homepage.newsletter.formSubheading}
						/>
					</div>
				</div>
			</section>
